...
var result = shell.exec('node -e \"qa-tools.log(1234);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
assert.ok(result.output === '1234\n' || result.output === '1234\nundefined\n'); // 'undefined' for v0.4

// check if stderr goes to output
var result = shell.exec('node -e \"qa-tools.error(1234);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
assert.ok(result.output === '1234\n' || result.output === '1234\nundefined\n'); // 'undefined' for v0.4

// check if stdout + stderr go to output
var result = shell.exec('node -e \"qa-tools.error(1234); qa-tools.log(666);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
assert.ok(result.output === '1234\n666\n' || result.output === '1234\n666\nundefined\n');  // 'undefined' for v0.4
...
var result = shell.exec( util.format('node -e "qa-tools.log(%s);"', "\\\"\\'+\\'_\\'+\\'\\\"") );
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
...
// no callback
var c = shell.exec('node -e \"qa-tools.log(1234)\"', {async:true});
assert.equal(shell.error(), null);
...
//
shell.exec('node -e \"qa-tools.log(5678);\"', function(code, output) {
  assert.equal(code, 0);
  ...
  //
  shell.exec('node -e \"qa-tools.log(5566);\"', {async:true}, function(code, output) {
    assert.equal(code, 0);
